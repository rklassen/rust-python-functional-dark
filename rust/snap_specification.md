...
- Unknown keys are not part of canonical `v0.6`.
...
- Matchpoints are typed half-edge endpoints and are valid in source and target
  positions.
- Builtins such as `get(@id)` are not standardized in `v0.6`; explicit named
...
- The format mark is a single letter from `{s, u, h}`, placed between `)`
  and `->`. The letters `i`, `f`, `b` are reserved for future use.
...
- Brackets `[` and `]` are not used in edge weights. A length-1 vector is
  equivalent to a scalar in the data model.
